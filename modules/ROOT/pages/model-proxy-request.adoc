...
:page-aliases: gateway::llm-proxy-request.adoc, gateway::flex-gateway-llm-proxy-request.adoc, general::model-proxy-request.adoc
...
https://developers.openai.com/api/reference/resources/chat[Chat Completions] (/chat/completions) is the standard API for interacting with models. It is designed for conversational multi-turn interactions rather than simple text continuation. The endpoint requires a model and a messages list with roles (such as system, user, or assistant) to generate context-aware responses.

These examples validate the basic functionality of the Chat Completions API.

==== Basic Chat Completion Example

This request example ensures the gateway correctly routes traffic to the specified LLM provider.
