= Troubleshoot the MuleSoft Agent
:keywords: ai assistant troubleshooting, ai assistant errors, incorrect answers, search results, mulesoft ai assistant, anypoint platform
:page-aliases: general::exp-ai-assistant-troubleshoot.adoc

If you experience issues using the MuleSoft Agent AI assistant, use this guide to identify and resolve common problems.

== MuleSoft Agent Doesn't Respond

If the MuleSoft Agent doesn't respond to your messages:

Check your connection:

. Verify that you have an active internet connection.
. Refresh the browser page.
. Try sending a simple message like "Hello" to test if the agent responds.
...
. Disable browser extensions that might interfere with the agent.

If the problem persists, contact your administrator or support team.

== MuleSoft Agent Gives Incorrect or Incomplete Answers

If the agent provides answers that don't match your expectations:
...
Verify the agent has access to the data:
...
== MuleSoft Agent Suggests Actions I Can't Perform

If the agent suggests actions but you can't perform them:

Check your permissions:

. Verify your Access Management permissions in xref:home-start.adoc#permissions[Enhanced Experience Permissions].
. Ask your administrator to review your role assignments.
. The agent may suggest actions available in the product, but your specific permissions determine what you can do.

Check your subscription tier:

. Verify whether you have API Portfolio Access or Agent + API Portfolio Access.
. Some features (like agents, MCP servers, Model proxies) require Agent + API Portfolio Access.
...
== MuleSoft Agent Is Slow to Respond

If the agent takes a long time to respond:
...
== MuleSoft Agent Conversation History Is Lost

If your agent conversation history disappears:
...
== Actions Performed by MuleSoft Agent Fail

If the agent confirms an action but the action fails:

Review error messages:

. The agent should display any error messages from the system.
...
. This helps determine whether the issue is with the agent or the underlying system.

== MuleSoft Agent Can't Access Recent Changes

If the agent doesn't recognize recently registered services or configuration changes:
...
+
See xref:agent-use.adoc[] for complete agent usage guidance.
...
* The exact question or request you sent to the agent
